...
#%%  VANE5   argus TGBT22A_603_05 (26 GB big dataset)   N2H+
...
f1 = dysh_data(accept="multibighuge")   # 2.3GB
print(f1)
sdf1 = GBTFITSLoad(f1, skipflags=True)   # about 4 sec
sdf1.get_summary()
sp1 = sdf1.getps(scan=51,ifnum=0,plnum=0,fdnum=0).timeaverage()
sp1[4000:14000].stats()

f2 = dysh_data(test="getps")   #  45MB
print(f2)
sdf2 = GBTFITSLoad(f2, skipflags=True)
sp2 = sdf2.getps(scan=51,ifnum=0,plnum=0,fdnum=0).timeaverage()
sp2[4000:14000].stats()

sp1.plot()
sp2.plot()

sdf1_docs =  """
8      7  2148+0657       0.0    Nod         1  34.638280  36.466260     3      2     16       2  160.0608    57.1523
9      8  2148+0657       0.0    Nod         2  34.638280  36.466260     3      2     16       2  160.3163    57.1967
10     9      AGPeg       0.0    Nod         1  34.638280  36.466260     3      2     31       2  156.4807    62.5263
11    10      AGPeg       0.0    Nod         2  34.638280  36.466260     3      2     31       2  157.0208    62.6167
"""

sdf1.write("nod_test.fits",scan=[7,8], ifnum=0, plnum=0, overwrite=True)
sdf1.write("nod_test.fits",scan=[7,8], ifnum=0, overwrite=True)
sdf4 = GBTFITSLoad("nod_test.fits")
sdf4["PLNUM"] = 0 
sdf4.getnod(scan=7,ifnum=0,plnum=0)


##%% issue 

f1 = dysh_data(accept="multibighuge")
sdf1 = GBTFITSLoad(f1, skipflags=True) 
sdf1.write("nod_test.fits",scan=[7,8], ifnum=0, overwrite=True)
sdf4 = GBTFITSLoad("nod_test.fits")
# sdf4.getnod(scan=7,ifnum=0,plnum=0)       # this fails
sdf4["PLNUM"] = 0     # patch them all to 0
sdf4.getnod(scan=7,ifnum=0,plnum=0)       # now it works


#%%

f1 = dysh_data(test="getps2")
sdf1 = GBTFITSLoad(f1)
sp1 = sdf1.getps(plnum=0,ifnum=0,fdnum=0).timeaverage()
sp2 = sdf1.getsigref(152,153, plnum=0,ifnum=0,fdnum=0).timeaverage()
